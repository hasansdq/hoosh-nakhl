...
Task ID: csv-tools-1
Agent: Z.ai Code (main)
Task: افزودن قابلیت درون‌ریزی و برون‌بری انبوه محصولات با CSV در پنل مدیریت — بخش جدید «ابزارها»، بدون دادهٔ تصویر در CSV، با فایل نمونه، همگام‌سازی حرفه‌ای رفت‌وبرگشت و UI فوق‌حرفه‌ای RTL (درخواست کاربر)

Work Log:
- **موتور مشترک CSV (src/lib/products-csv.ts — کلاینت+سرور):** پارسر RFC 4180 (نقل‌قول، کاما داخل فیلد، خط جدید، CRLF) + تشخیص خودکار جداکننده (کاما/سمی‌کالن/تب برای Excel فارسی) + حذف BOM؛ سرستون‌های رسمی ۱۵گانه فارسی + نام‌های جایگزین فارسی/انگلیسی (تطبیق با normalizePersian + squish بدون فاصله — برچسب رسمی هم alias است تا رفت‌وبرگشت خروجی→ورودی همیشه بخندد)؛ اعداد: ارقام فارسی/عربی + جداکنندهٔ هزارگان (, و ٬)؛ بولین: بله/خیر/yes/no/true/false/1/0/فعال/غیرفعال/دارد/ندارد/✓/✗؛ اعتبارسنجی ردیف‌محور با پیام فارسی (قیمت ۱٬۰۰۰–۵۰۰م، نام ۲–۸۰، توضیحات/مواد ۶۰۰، کالری ۰–۵۰۰۰، آماده‌سازی ۰–۶۰۰، ترتیب ۰–۹۹۹)؛ ردیابی fieldsPresent: ستون غایب ← فیلد در به‌روزرسانی دست‌نخورده، سلول خالی ← پاک/پیش‌فرض؛ محدودیت ۲MB/۵۰۰۰ ردیف؛ notes سطح‌فایل (تعداد سفارش فقط‌خواندنی)
- **API برون‌بری GET /api/admin/tools/products/export:** همهٔ اطلاعات غیرتصویری + ستون «شناسه» (رفت‌وبرگشت بدون تکرار)؛ BOM؛ نام فایل با تاریخ شمسی (nakhl-products-1405-06-24.csv)؛ audit MENU_CSV_EXPORT
- **API فایل نمونه GET .../template:** ۳ ردیف نمونه (کامل/حداقلی/اعداد فارسی) با نام دسته‌بندی‌های واقعی از DB؛ audit MENU_CSV_TEMPLATE
- **API درون‌ریزی POST .../import (multipart یا JSON):** دو حالت — sync (به‌روزرسانی با شناسه → سپس نام+دسته → وگرنه ایجاد) و create-only (رد تکراری‌ها)؛ گزینهٔ ساخت خودکار دستهٔ جدید (slug یکتا cat-* بدون Buffer — سازگار با هر دو ران‌تایم)؛ نتایج سطری {row,name,action,message} با شمارنده‌ها + categoriesCreated + durationMs؛ هشدار شناسهٔ یافت‌نشده/لمس دوبارهٔ محصول در همان فایل؛ **imageUrl و gallery هرگز نوشته نمی‌شوند**؛ orderCount هرگز تغییر نمی‌کند؛ audit MENU_CSV_IMPORT
- **UI — ToolsManager.tsx + تب «ابزارها» (Wrench) در AdminPanel/admin-store:** سرتیتر با بج تعداد محصول/دسته (لحظه‌ای)؛ دو کارت برون‌بری/فایل نمونه (گرادیان سبز/طلایی)؛ درون‌ریزی: ناحیهٔ drag&drop (کیبورد هم قابل دسترس) → پیش‌نمایش با همان موتور مشترک (شمارنده‌های کل/معتبر/نامعتبر/ستون، هشدار ستون ناشناخته/تکراری + یادداشت سطح‌فایل، فهرست خطاهای ردیف‌ها با اسکرول، جدول پیش‌نمایش ۸ ردیف معتبر با قیمت تومانی) → انتخاب حالت (radio card) + سوییچ ساخت دسته → اجرا → پنل گزارش (۴ شمارندهٔ رنگی، دسته‌های ساخته‌شده، جزئیات همهٔ ردیف‌ها با بج وضعیت، دکمهٔ به‌روزرسانی آمار/درون‌ریزی فایل دیگر)؛ آکاردئون راهنمای کامل ۱۵ ستون + ۶ قاعدهٔ همگام‌سازی (حفظ تصاویر، رفت‌وبرگشت، ستون غایب/سلول خالی، سازگاری Excel، اعتبارسنجی، بدون حذف خودکار)
- **باگ‌های رفع‌شده در طول کار:** (۱) برچسب «قیمت (تومان)» در aliasها نبود → matchHeader حالا اول برچسب‌های رسمی را می‌سنجد؛ (۲) هشدار «تعداد سفارش» برای هر ردیف تکرار می‌شد → منتقل به notes سطح‌فایل؛ (۳) «تغییر فایل» value قبلی input را پاک می‌کند تا انتخاب مجدد همان فایل ممکن شود؛ (۴) نمایش حجم با ارقام فارسی
- **تست‌های عملی (همه سبز):** tsc صفر · lint صفر · curl: export ۳۰ محصول/BOM/سرستون درست · template با دسته‌های واقعی · import رفت‌وبرگشت ۳۳ ردیف: ۳۰ به‌روزرسانی (شناسه) + ۲ ایجاد (یکی با دستهٔ جدید خودکار) + ۱ خطای قیمت — قیمت «۱۹۹٬۰۰۰» فارسی درست پارس شد، توضیح با کاما/سمی‌کالن رفت‌وبرگشت شد، **imageUrl+gallery دست‌نخورده ماند** · ردیف نامعتبر اجرا نشد · agent-browser E2E: ورود → تب ابزارها → آپلود CSV → پیش‌نمایش (۲ کل/۱ معتبر/۱ نامعتبر/۱۵ ستون + خطای ردیف ۲) → شروع درون‌ریزی → گزارش (۱ به‌روزرسانی/۱ ناموفق در ۰.۱ ثانیه) → دانلود export از مرورگر ✓ · VLM دو بار: «RTL درست، بدون گلچ، آمادهٔ پروداکشن» · داده‌های تست پاک شد (بازگشت به ۳۰ محصول/۷ دسته) · dev.log بدون خطای جدید · /api/health ok
- کامیت: `ffdc715` — همهٔ فایل‌ها ترک‌شده در Git (git ls-files تأیید؛ هیچ الگوی .gitignore مسیر tools را نمی‌گیرد)

Stage Summary:
- ✅ قابلیت کامل درون‌ریزی/برون‌بری CSV محصولات در تب جدید «ابزارها» با UI فوق‌حرفه‌ای RTL — بدون تغییر سایر بخش‌ها
- ✅ دیتای تصویر (imageUrl/gallery) نه در CSV هست نه هنگام درون‌ریزی دست می‌خورد؛ «تعداد سفارش» فقط اطلاعاتی/نادیده
- ✅ همگام‌سازی دقیق: شناسه → نام+دسته → ایجاد؛ ستون غایب=حفظ فیلد، سلول خالی=پاک/پیش‌فرض؛ خروجی→ویرایش→ورودی بدون تکرار
- ✅ فایل نمونه استاندارد با دسته‌بندی‌های واقعی؛ پشتیبانی کاما/سمی‌کالن/تب + BOM + ارقام فارسی + جداکنندهٔ هزارگان
...
---
Task ID: baran-integration-1
Agent: Z.ai Code (main)
Task: اتصال کامل سایت به نرم‌افزار حسابداری باران طبق مستندات Baran_OnlineShopApi_V5.0.pdf — (۱) ارسال سفارش‌های سایت به باران (۲) دریافت محصولات از باران و پر شدن خودکار منو (۳) تضمین عدم ارسال مجدد + مدیریت اتصال در پنل ادمین

Work Log:
- **مطالعهٔ کامل مستندات** (upload/Baran_OnlineShopApi_V5.0.pdf — ۱۰ صفحه): ۷ متد تعریف‌شده؛ طبق درخواست کاربر فقط ۴ متد موردنیاز پیاده شد: ProductSEND و SENDPics (باران→سایت) و Orders و ClearOrders (سایت→باران) — CustomersSEND/SendCoupons/GetChargeWallet عمداً خارج از scope (پاسخ 404 توصیفی می‌دهند)
- **اسکیما + مایگریشن 0003_baran_sync.sql:** فیلدهای baranProductId/baranPictureName/baranUnit/baranSyncedAt روی MenuItem · baranGroupId/baranMainGroupId روی Category · baranFactorNumber/baranSentAt روی Order · baranCustomerId روی User (>۵۰۰۰۰۰ طبق الزام مستندات) · دو جدول جدید BaranSyncLog و BaranAsset — اعمال روی D1 لوکال + prisma generate؛ Docker migrate.mjs و wrangler d1 migrations هر دو خودکار 0003 را برمی‌دارند
- **src/lib/baran.ts (هستهٔ یکپارچه‌سازی):** انواع C# با PascalCase دقیق + تحمل «ProductName٢» (رقم عربی مستندات) · نگات ثابت SHA-256 برای کلید · نرمال‌سازی ارقام فارسی/جداکننده‌ها · sniff نوع تصویر از magic-bytes (نه نام فایل!) · زمان تهران +03:30 · تبدیل موبایل 98→09 · سقف ۳۰۰ قلم در هر فراخوانی (مازاد StatusId=0 → باران دوباره می‌فرستد)
- **ProductSEND:** upsert با **فرخوانی (adoption)** — دسته/آیتم هم‌نامِ متصل‌نشده به باران لینک می‌شود و تکراری ساخته نمی‌شود (ایراد واقعی تست اول: «کباب‌ها»ی دومی ساخته شده بود → ریشه‌ای حل شد) · ChangeType=2 → مخفی‌سازی (حذف فیزیکی ممنوع — حفظ تاریخ سفارش‌ها) · قیمت نهایی = SellPrice×(1−DiscountPrecent/100) · ProductStatus 2/3 → isSpecial · IgnoreStock/RemainCount → isAvailable · **غیبت فیلد در ویرایش یعنی «بدون تغییر»** (PictureName/انبار/قیمت — سه باگ بالقوه در تست کشف و رفع شد، مهم‌ترین: baranProductId در data جا افتاده بود و لینک هرگز ذخیره نمی‌شد!)
- **SENDPics:** ذخیره از طریق saveImageUpload (dual-backend R2/fs همان آپلودهای عادی) + جدول BaranAsset با تطبیق PicFileName ↔ baranPictureName در هر دو جهت (تصویر قبل یا بعد از کالا برسد وصل می‌شود) — تست با PNG واقعی + فایل خراب (رد دقیق سطری)
- **Orders (GET):** فقط سفارش‌های baranSentAt=NULL و در وضعیت پرداخت‌شده (قابل گسترش به در انتظار با تنظیم) · تخصیص پایدار FactorNumber و CustomerId (500001+) فقط بار اول · خروجی کامل و بدون هیچ null: TotalAmount=subtotal، DiscountAmount=کد تخفیف، SendAmount=هزینه ارسال، ServiceAmount=مالیات (جمع اقلام+ارسال+خدمات=FinalAmount دقیق)، Comment=نوت+نوع تحویل، PaymentCode 1/0، CustomerSex از gender، VariationID="0" · آیتم‌ها با ProductId واقعی باران (کالای بدون اتصال → 0)
- **ClearOrders:** ItemType=2 → مهر baranSentAt (تضمین عدم ارسال مجدد — at-least-once + ack صریح) · ItemType=5 (کیف پول) → no-op موفق · تحمل ارقام فارسی و جداکننده‌های - , ; فاصله
- **پنل ادمین — تب جدید «اتصال باران» (BaranManager.tsx):** کارت وضعیت با سوییچ فعال/غیرفعال + ۵ آمار زنده · راهنمای پیکربندی (آدرس API + کلید با کپی/نمایش/تولید مجدد با AlertDialog) · ۴ متد با badge فارسی · تنظیمات (سطح دسته‌بندی زیرگروه/سرگروه، الزام کلید، موجودی از باران، مخفی‌سازی حذف‌شده‌ها، سفارش‌های در انتظار) با dirty-state · جدول اقلام متصل (کد کالا/واحد/قیمت/وضعیت/تصویر/زمان شمسی) · جدول لاگ ۳۰ فراخوانی · عملیات پیشرفته: «ارسال مجدد سفارش‌ها» و «منوی کاملاً بارانی» (مخفی‌سازی اقلام دستی — غیرمخرب و قابل بازگشت)
- **APIهای ادمین:** GET/PUT /api/admin/baran (whitelist دقیق؛ فعال‌سازی بدون کلید → تولید خودکار) + rotate-key + resend-orders + hide-manual — همه با AuditLog
- **رفع اشکالات محیط سندباکس:** Turbopack روت‌های ساخته‌شده حین اجرا را 404 می‌داد → ترفند rename برای re-scan · کلاینت Prisma کش‌شده در حافظه → ری‌استارت درخت پروسهٔ واقعی (dev.pid قدیمی بود) · فایل‌های types تولیدی خراب → حذف و تولید مجدد · خطای pre-existing تایپ dir در AuthModal هم رفع شد
- **E2E کامل با شبیه‌سازی curl باران:** 401 بدون/با کلید غلط · 404 متد ناشناخته · 405 متد غلط · ProductSEND: ۳ موفق + ۱ رد (نام خالی) با پیام فارسی، فرخوانی کباب کوبیده (185000→166500 با تخفیف ۱۰٪، ویژه، بدون تکرار دسته/آیتم)، ایجاد خورشت و دوغ، ویرایش قیمت→200000، حذف دوغ→ناموجود · SENDPics: 2 موفق + 1 رد، PNG sniff درست (نام jpg ولی محتوا PNG)، ghormeh لینک دوسویه · Orders: فاکتور ۱ با همهٔ فیلدها (مشتری 500001، تلفن 0913…، زمان تهران، سفارش PENDING_PAYMENT مستثنی) · ClearOrders: تأیید → GET بعدی خالی (ایدمپتانسی) · ارقام فارسی در ItemIds · resend-orders ادمین → سفارش برگشت · hide-manual: ۲۹ قلم + ۶ دسته (برای پیش‌نمایش بازیابی شد)
- **QA مرورگر (agent-browser):** تب باران کامل رندر شد — آمار واقعی، جدول اقلام (واحد «(بطری)» کنار نام)، لاگ‌ها با زمان شمسی، نمایش/کپی کلید، سوییچ الزام کلید + ذخیره (تأیید server-side: بدون کلید 200 → بعد از فعال‌سازی 401) · صفحهٔ اصلی: کباب کوبیده ۲۰۰,۰۰۰ تومان با توضیح باران و تصویر قبلی + تب جدید «خورشت‌ها» · صفر خطای کنسول/صفحه · اسکرین‌شات دسکتاپ و موبایل
- رگرسیون: lint صفر · tsc صفر

Stage Summary:
- ✅ اتصال باران کامل و طبق مستندات V5.0: آدرس پایه /api/ApiServiceBaran/ + ۴ متد (case-insensitive) با auth اختیاری-but-default-ON و کلید در ۳ کانال
- ✅ منو از باران پر می‌شود با adoption هوشمند (بدون تکراری)، تصاویر دوطرفه لینک می‌شوند، حذف=مخفی، موجودی از انبار باران
- ✅ سفارش‌ها only-once با ack صریح (ClearOrders)؛ FactorNumber/CustomerId پایدار و طبق قواعد مستندات (>۵۰۰۰۰۰)؛ همهٔ خروجی‌ها non-null
- ✅ مدیریت کامل در پنل ادمین: فعال‌سازی، کلید، تنظیمات رفتار، آمار، لاگ، ارسال مجدد، منوی کاملاً بارانی
- ⚠️ دقت: اولین sync بعدی باران اقلام جدید را ارسال می‌کند؛ اگر باران فکر می‌کند قبلاً فرستاده (StatusId=1 گرفته) ولی سایت داده را ندارد، از پنل → «ارسال مجدد» فقط برای سفارش است؛ برای محصولات باید در خود باران reset شود
- بک‌لاگ قبلی + پیشنهاد آینده: CustomersSEND/SendCoupons اگر روزی لازم شد، Web Push، نظرات عمومی، CSV دسته‌ها

---
Task ID: baran-cms-status-1
Agent: Z.ai Code (main)
Task: بررسی و اصلاح کامل «مدیریت اتصال به باران و وضعیت آن» در پنل CMS

Work Log:
- بررسی وضعیت: تب «اتصال باران» در سندباکس موجود و سالم بود (کامیت 5c810cd)؛ ریشهٔ گزارش کاربر به احتمال زیاد نسخهٔ قدیمی روی VPS است (پیش از 5c810cd هیچ تب بارانی وجود نداشت) — اما «وضعیت اتصال» واقعاً ناقص بود: فقط سوییچ فعال/غیرفعال بدون هیچ سلامت واقعی
- نتیجهٔ ممیزی نواقص: (۱) هیچ سنجه‌ای از تماس واقعی باران (آخرین تماس/آخرین موفقیت/آخرین خطا/نرخ موفقیت) وجود نداشت (۲) هیچ ابزار تستی برای بررسی سلامت مسیر باران وجود نداشت (۳) داشبورد اصلی هیچ دیدی از وضعیت باران نداشت
- Backend: متد GET Ping غیرمخرب و guarded به ApiServiceBaran اضافه شد (case-insensitive، همان baranGuard: 503 غیرفعال/401 کلید؛ پاسخ {ok,service,method,serverTime} با no-store) + پیام متد ناشناخته به‌روز شد
- Backend: GET /api/admin/baran حالا health کامل برمی‌گرداند — computeHealth از BaranSyncLog: verdict (off/no_contact/healthy/stale/failing)، lastCallAt، lastSuccessAt، lastErrorAt+Message+Method، calls24h/calls7d، okCalls/failedCalls، successRate (پنجرهٔ ۳۰ لاگ اخیر)
- Backend جدید: POST /api/admin/baran/test — تست سرتاسری اتصال با شبیه‌سازی دقیق فراخوانی باران از بیرون: (۱) انتخاب origin از x-forwarded-proto/host با fallback به origin خود سرور (۲) دسترسی endpoint (۳) الزام کلید بدون کلید (۴۰۱=pass) (۳) رد کلید غلط (۴۰۱=pass) (۴) round-trip با کلید معتبر (200+ok=pass) — هر گام با ms، حکم کلی healthy/issues/broken، ثبت در AuditLog (BARAN_CONNECTION_TEST)، بدون نوشتن در BaranSyncLog (تمیز ماندن لاگ باران) و بدون هیچ تغییر داده
- رفع باگ منطقی حین QA: وقتی اتصال غیرفعال بود، گام «کلید غلط» هم 503 می‌گرفت و پیام گمراه‌کنندهٔ «نشت احتمالی» می‌داد → حالا با 503 گام‌های کلید skip می‌شوند («تا فعال‌شدن اتصال، این گام معنایی ندارد»)
- Frontend BaranManager: کارت «وضعیت اتصال» → «وضعیت و سلامت اتصال» بازطراحی شد: نشان حکم با نقطهٔ تپندهٔ رنگی (سبز/کهربایی/قرمز/خاکستری) + توضیح فارسی، ۴ متریک تماس (آخرین تماس باران، آخرین همگام‌سازی موفق — با sinceLabel نسبی فارسی، فراخوانی ۲۴h/۷d، نرخ موفقیت رنگی)، Alert قرمز آخرین خطا با متد و زمان، آمار قبلی، و پنل «تست اتصال» با دکمه + نتیجهٔ هر گام (آیکون pass/fail/warn/skip + ms) + URL endpoint تست‌شده؛ متد Ping به فهرست «متدهای فعال» اضافه شد
- Frontend AdminDashboard: ویجت جدید BaranStatusWidget زیر کارت‌های آماری — حکم سلامت با نقطهٔ تپنده، آخرین تماس/اقلام همگام/سفارش در انتظار، دکمهٔ «مدیریت اتصال» با setTab("baran")؛ در خطای API بی‌سروصدا مخفی می‌شود
- QA کامل: lint صفر · tsc صفر · agent-browser: ورود، ویجت داشبورد («متصل و سالم — آخرین تماس باران: ۱ ساعت پیش»)، پرش به تب، کارت سلامت کامل، تست اتصال در حالت فعال (۴ گام سبز «کاملاً سالم»)، خاموش‌کردن (حکم «غیرفعال» + تست «ناسالم» با گام‌های skip درست)، روشن‌کردن مجدد (سالم) · curl: ping بدون کلید 401، متد ناشناخته 404، pInG case-insensitive 401، admin health=healthy · VLM اسکرین‌شات موبایل 390px و دسکتاپ 1280px: بدون سرریز/هم‌پوشانی · dev.log فقط پاسخ‌های موردانتظار (هشدار DO مربوط به wrangler local است و pre-existing) · صفر خطای کنسول

Stage Summary:
- ✅ «مدیریت اتصال و وضعیت باران» حالا کامل است: حکم سلامت زنده از فراخوانی‌های واقعی باران + تست سرتاسری شبیه‌سازی‌شدهٔ مسیر باران + دید در داشبورد اصلی
- ✅ متد Ping عمومی: ابزار سلامت غیرمخرب برای تست و عیب‌یابی (نیازی نیست باران صدایش بزند ولی اگر زد هم بی‌ضرر است)
